...
public class GccVersionDeterminer implements CompilerMetaDataProvider {
    ...
    private GccVersionResult transform(String output, String error, File gccBinary) {
        ...
        return new DefaultGccVersionResult(versionNumber, architecture, systemIncludes, clang);
    }
    ...
    private Map<String, String> parseDefines(String output, File gccBinary) {
        ...
        try {
            while ((line = reader.readLine()) != null) {
                ...
                if (!matcher.matches()) {
                    throw new BrokenResultException(String.format("Could not determine %s version: %s produced unexpected output.", getDescription(), gccBinary.getName()));
                }
    ...
    private static class DefaultGccVersionResult implements GccVersionResult {
        ...
        public DefaultGccVersionResult(VersionNumber scrapedVersion, ArchitectureInternal architecture, ImmutableList<File> systemIncludes, boolean clang) {
